...
Region: eu-west-2. Domain: mahadvo.com → it-tools.mahadvo.com. Flow: client hits ALB (443 HTTPS), which forwards to the target group (3000), which routes to the ECS Fargate task. Modules: VPC, IAM, ACM, ALB, ECR, ECS, CloudWatch. There's no separate Route53 module. That lives inside ACM (zone lookup, cert validation) and ALB (the alias record).
...
- **ECR**: looks up an existing repo and the S3 prefix list via `data` (repo isn't created by this code).
- **ECS**: two security groups (task + endpoint) linked via explicit `security_group_rule` resources to avoid a cycle. Same rule covers app traffic and ALB health checks since they're on the same port. Four VPC endpoints: ECR API, ECR DKR, Logs, S3. `runtime_platform` set to `X86_64` since GitHub Actions runners are x86_64, not ARM. Service points at `.arn`, not `.family`, to avoid state drift.
- **CloudWatch**: log group plus CPU/memory alarms (≥80%, 2×180s). No SNS wired up yet, so no actual notifications.
- **ALB**: 443 open to the world, health-checks `/health` on 3000, HTTPS listener depends on ACM validation, Route53 alias record routes traffic.
...
- `AmazonECSTaskExecutionRolePolicy`, an AWS-managed IAM policy, via `data "aws_iam_policy" "ecs_task_execute_policy"` in the IAM module
